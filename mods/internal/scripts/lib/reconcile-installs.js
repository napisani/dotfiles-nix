...
function reconcileInstalls(
  ...
  if (mode === "status") {
    try {
      ...
      let ok = true;
      for (
        const id of new Set([...Object.keys(entries), ...Object.keys(desired)])
      ) {
        ...
        try {
          ...
          if (spec === null) status = "removal pending";
          else if (status === "healthy") {
            if (!previous) status = "untracked";
            else if (previous.pending) status = "retry pending";
            else if (
              previous.spec === null ||
              signature(previous.spec) !== signature(spec)
            ) status = "declaration changed";
          }
        } catch (error) {
