...
test("status reports drift without creating state or invoking installers", (t) => {
  const h = harness(t);
  assert.equal(h.run({ mode: "status" }), false);
  assert.deepEqual(h.calls, []);
  assert.equal(fs.existsSync(h.options.stateFile), false);
  assert.equal(fs.existsSync(h.options.stateFile + ".lock"), false);
  h.run();
  h.calls.length = 0;
  const before = fs.statSync(h.options.stateFile).mtimeMs;
  assert.equal(h.run({ mode: "status" }), true);
  assert.equal(
    h.run({ mode: "status", desired: { a: { version: 2 } } }),
    false,
  );
  assert.deepEqual(h.calls, []);
  assert.equal(fs.statSync(h.options.stateFile).mtimeMs, before);
