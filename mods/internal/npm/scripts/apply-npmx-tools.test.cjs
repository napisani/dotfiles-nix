...
test("installation fixes only the package's executable permissions", (t) => {
  const h = fixture(t);
  fs.mkdirSync(path.join(h.dir, "bin"));
  const manual = path.join(h.dir, "bin/manual");
  fs.writeFileSync(manual, "manual", { mode: 0o600 });
  const result = h.run(undefined, { NPM_NO_EXEC: "1" });
  assert.equal(result.status, 0, result.stdout + result.stderr);
  assert.equal(fs.statSync(manual).mode & 0o777, 0o600);
  h.clear();
  assert.equal(h.run().status, 0);
  assert.deepEqual(h.calls(), []);
});
test("status is read-only even with missing artifacts and forced repair", (t) => {
  const h = fixture(t);
  assert.equal(
