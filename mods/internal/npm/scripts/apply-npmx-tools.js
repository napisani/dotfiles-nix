...
try {
  ...
  const ok = reconcileInstalls({
    ...
    install(name, spec) {
      ...
      // Some packages omit executable bits. Repair only this package's own
      // linked bins, never chmod the entire shared ~/.local/bin tree.
      const root = fs.realpathSync(path.join(prefix, "lib/node_modules", name));
      ...
      const bins = typeof pkg.bin === "string"
        ...
        : pkg.bin || {};
      for (const [bin, target] of Object.entries(bins)) {
        const file = fs.realpathSync(path.join(root, target));
        if (
          !file.startsWith(root + path.sep) ||
          fs.realpathSync(path.join(prefix, "bin", bin)) !== file
        ) throw new Error(`unexpected executable target: ${bin}`);
        const mode = fs.statSync(file).mode;
        if (!(mode & 0o100)) fs.chmodSync(file, mode | 0o500);
      }
    },
