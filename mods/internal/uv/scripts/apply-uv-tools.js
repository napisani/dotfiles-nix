// uv adapter: registry tools and editable toolbox projects.
// Env: DECLARED_TOOLS={name:{package,with?,extras?}}, TOOLBOX,
...
try {
  ...
  const ok = reconcileInstalls({
    ...
    observe(id, spec, previousSpec) {
      // Removal-only migration for previously tracked Vocal dependencies.
      // New generations provide its Python through Home Manager instead.
      // Reviewed 2026-09-06: retire after all hosts + rollback window migrate;
      // see docs/contracts/global-tools-migrations.md. No ledger ID => no probe.
      if (id === "venv:vocal") {
        const venv = previousSpec;
        ...
        return {
          status: venv.packages.some(present) ? "healthy" : "missing",
        };
      }
