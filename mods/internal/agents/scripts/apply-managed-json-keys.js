...
if (!exists) {
  ...
  try {
    ...
    console.error(
      "agents: refusing to update invalid JSON at " + targetFile + ": " +
        e.message,
    );
    process.exit(1);
  }
  if (typeof config !== "object" || config === null || Array.isArray(config)) {
    console.error(
      "agents: refusing to update " + targetFile +
...
if (current !== next) {
  if (writeConfigFile(targetFile, next)) {
    console.log(
      "agents: applied managed '" + managedKey + "' entries -> " + targetFile,
    );
  }
}
