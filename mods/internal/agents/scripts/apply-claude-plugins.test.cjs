...
test("first convergence registers marketplaces and installs declared plugins", () => {
  ...
  const result = run(dir, {
    CALL_LOG: log,
    ...declarations(["plugin@marketplace"], state),
  });

  assert.equal(result.status, 0, result.stderr);
  const calls = readCalls(log);
  assert.ok(
    calls.includes("plugin marketplace add owner/marketplace --scope user"),
  );
  assert.ok(calls.includes("plugin install plugin@marketplace --scope user"));
  assert.equal(
    run(
      dir,
      declarations(["plugin@marketplace"], state, { RECONCILE_MODE: "status" }),
    ).status,
    0,
  );
});
...
test("explicit update refreshes marketplaces and updates plugins", () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  const calls = readCalls(log);
  assert.ok(calls.includes("plugin marketplace update marketplace"));
...
test("a failed install records non-convergence and retries", () => {
  ...
  assert.equal(run(dir, { ...env, RECONCILE_MODE: "status" }).status, 1);
...
test("a failed removal retains ownership so pruning is retried", () => {
  ...
  assert.equal(failed.status, 1);
  assert.equal(
    run(dir, declarations([], state, { RECONCILE_MODE: "status" })).status,
    1,
  );
...
test("status never installs missing plugins or registers marketplaces", () => {
  const dir = mkTmpDir();
  const state = path.join(dir, "state.json");
  const result = run(
    dir,
    declarations(["plugin@m"], state, { RECONCILE_MODE: "status" }),
  );
  assert.equal(result.status, 1);
  assert.deepEqual(readCalls(path.join(dir, "calls.log")), [
    "plugin list --json",
  ]);
  assert.equal(fs.existsSync(state), false);
});
test("unknown inventory fails closed; explicit repair only attempts declared installs", () => {
  const dir = mkTmpDir();
  const state = path.join(dir, "state.json");
  const env = declarations(["plugin@m"], state, { CLAUDE_LIST_INVALID: "1" });
  assert.equal(run(dir, env).status, 1);
  assert.deepEqual(readCalls(path.join(dir, "calls.log")), [
    "plugin list --json",
  ]);
  fs.writeFileSync(path.join(dir, "calls.log"), "");
  // Still non-converged until native inventory can validate the result.
  assert.equal(run(dir, { ...env, FORCE_REPAIR: "1" }).status, 1);
  const calls = readCalls(path.join(dir, "calls.log"));
  assert.ok(calls.includes("plugin install plugin@m --scope user"));
  assert.ok(!calls.some((call) => call.startsWith("plugin uninstall")));
});
test("corrupt state is preserved and does not authorize pruning", () => {
  ...
  assert.ok(
    !readCalls(log).includes("plugin uninstall unmanaged@m --scope user"),
  );
});
