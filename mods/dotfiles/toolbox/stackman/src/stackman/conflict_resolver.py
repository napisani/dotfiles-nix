...
class RebaseConflictResolution:
    ...
    def _try_interactive(self) -> ConflictResolutionResult:
        ...
        while True:
            ...
            if line == "":
                self.ctx.stderr.write("[stackman] Input closed while waiting for rebase resolution.\n")
                return ConflictResolutionResult(
                    status="failure",
                    message="Input closed while waiting for rebase resolution",
                )
            if self.validator.is_rebase_in_progress():
                _emit(self.ctx, f"[stackman] Rebase on {self.conflict_ctx.branch_name!r} is still in progress.")
                continue
            if self.validator.is_rebase_complete():
                _emit(self.ctx, f"[stackman] Rebase on {self.conflict_ctx.branch_name!r} completed; resuming sync.")
                return ConflictResolutionResult(
                    status="success",
                    message="Rebase completed successfully (interactive)",
                )
            self.ctx.stderr.write(f"[stackman] Rebase on {self.conflict_ctx.branch_name!r} was aborted.\n")
            return ConflictResolutionResult(
                status="needs_manual",
    ...
    def _try_resolver(self) -> ConflictResolutionResult:
        ...
        return _invoke_resolver(self.ctx, self.conflict_ctx, self.resolver)
