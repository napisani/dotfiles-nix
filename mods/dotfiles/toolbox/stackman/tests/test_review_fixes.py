...
def test_merge_repo_records_moves_and_dedupes_branches(tmp_path) -> None:
    ...
    with connect(db) as conn:
        ...
        assert conn.execute("SELECT COUNT(*) c FROM repos WHERE id=?", (victim,)).fetchone()["c"] == 0
        names = {r["branch_name"] for r in conn.execute("SELECT branch_name FROM branches WHERE repo_id=?", (survivor,))}
        assert names == {"feature", "other"}
        # The colliding 'feature' was deduped, not duplicated.
        assert conn.execute("SELECT COUNT(*) c FROM branches WHERE branch_name='feature'").fetchone()["c"] == 1
...
def test_retracking_interior_branch_keeps_descendants_in_one_stack(git_repo, stackman_db_path) -> None:
