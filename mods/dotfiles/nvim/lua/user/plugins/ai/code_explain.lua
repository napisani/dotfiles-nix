...
local function cleanup_request(bufnr)
  local job = M._pending_requests[bufnr]
  if job then
...
local function extract_context(range)
  ...
  return {
    ...
    filename = fn.fnamemodify(api.nvim_buf_get_name(bufnr), ":.") ~= "" and fn.fnamemodify(api.nvim_buf_get_name(bufnr), ":.")
      or "[No Name]",
...
local function start_request(bufnr, context)
  ...
  resolved_adapter.opts = vim.tbl_deep_extend("force", { stream = false }, resolved_adapter.opts or {})
  ...
  local handle = http_client
    .new({ adapter = resolved_adapter })
    :request(payload, {
  ...
  if handle then
    M._pending_requests[bufnr] = handle
