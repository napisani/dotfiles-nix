...
local function prepare_search_replace(cmd, cursor_left, copy)
	return function()
		if copy then
...
local common_search = {
	...
	{ "<leader>rQ", prepare_search_replace(":cdo %s@@@gc", 4), desc = "(Q)uicklist ask" },
	{ "<leader>rd", prepare_search_replace(":g@@d", 2), desc = "(d)elete" },
	{ "<leader>rl", prepare_search_replace(":s@@@g", 3), desc = "(l)line" },
	{ "<leader>rq", prepare_search_replace(":cdo %s@@@g", 3), desc = "(q)uicklist" },
}
...
local visual_mappings = utils.extend_lists(
	replace_group,
	{
		...
		{
			"<leader>*rQ",
			prepare_search_replace(":cdo %s@<C-R>=expand('<cword>')<CR>@@gc", 3),
			desc = "(Q)uicklist ask",
		},
		...
		{
			"<leader>*rq",
			prepare_search_replace(":cdo %s@<C-R>=expand('<cword>')<CR>@@g", 2),
			desc = "(q)uicklist",
		},

		{ "<leader>rB", prepare_search_replace(":%s@@@gc", 4, true), desc = "(B)uffer ask" },
		{ "<leader>rQ", prepare_search_replace(":cdo %s@@@gc", 4, true), desc = "(Q)uicklist ask" },
		{ "<leader>rb", prepare_search_replace(":%s@@@g", 3, true), desc = "(b)uffer" },
		{ "<leader>rl", prepare_search_replace(":s@@@g", 3, true), desc = "(l)line" },
		{ "<leader>rq", prepare_search_replace(":cdo %s@@@g", 3, true), desc = "(q)uicklist" },
