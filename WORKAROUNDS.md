...
| **Revisit when** | Upstream bumps `claude-agent-sdk-pi`'s peer range off of `pi-ai@^0.74.0`. Check with `npm view claude-agent-sdk-pi peerDependencies`; re-add `"npm:claude-agent-sdk-pi"` to `agents.pi.packages` once it's compatible with the `pi-ai` version the other declared extensions need. |
