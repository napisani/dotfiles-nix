...
{ inputs, lib, config, pkgs, user, overlays, ... }: {
  ...
  programs.bash = {
    shellAliases = {
      nixswitchup =
        "pushd ~/.config/home-manager; git pull && darwin-rebuild switch --show-trace --flake ~/.config/home-manager/.# ; popd";
      nixswitch =
        "pushd ~/.config/home-manager; darwin-rebuild switch --show-trace --flake ~/.config/home-manager/.# ; popd";
